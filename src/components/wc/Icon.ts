const ICON_PATHS = {
  computer:
    "M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25",
  sun:
    "M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z",
  moon:
    "M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z",
  language:
    "m10.5 21 5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 0 1 6-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 0 1-3.827-5.802",
  "color-theme":
    "M9.53 16.122a3 3 0 0 0-5.78 1.128 2.25 2.25 0 0 1-2.4 2.245 4.5 4.5 0 0 0 8.4-2.245c0-.399-.078-.78-.22-1.128Zm0 0a15.998 15.998 0 0 0 3.388-1.62m-5.043-.025a15.994 15.994 0 0 1 1.622-3.395m3.42 3.42a15.995 15.995 0 0 0 4.764-4.648l3.876-5.814a1.151 1.151 0 0 0-1.597-1.597L14.146 6.32a15.996 15.996 0 0 0-4.649 4.763m3.42 3.42a6.776 6.776 0 0 0-3.42-3.42",
  settings:
    "M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75",
  bar3:
    "M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5",
  "check-circle":
    "M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z",
  "adjustments-vertical":
    "M6 13.5V3.75m0 9.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 3.75V16.5m12-3V3.75m0 9.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 3.75V16.5m-6-9V3.75m0 3.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 9.75V10.5",
  "arrow-down-on-square": "M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3",
  close: "M6 18 18 6M6 6l12 12",
  "exclamation-triangle": "M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z"
} as const;

type IconType = keyof typeof ICON_PATHS;

const ICON_ELEMENT_NAME = 'rl-icon';
const SVG_NAMESPACE = 'http://www.w3.org/2000/svg';

function isIconType(value: string | null): value is IconType {
  return !!value && Object.prototype.hasOwnProperty.call(ICON_PATHS, value);
}

export class IconElement extends HTMLElement {
  static observedAttributes = [
    'type',
    'class',
    'is-decorative',
    'is-standalone',
    'is-link-wrapped',
    'title',
  ];

  connectedCallback(): void {
    this.render();
  }

  attributeChangedCallback(): void {
    if (this.isConnected) {
      this.render();
    }
  }

  private render(): void {
    const type = this.getAttribute('type');

    this.replaceChildren();

    if (!isIconType(type)) {
      console.warn(`IconElement: "${type ?? ''}" is not a supported icon type.`);
      return;
    }

    const isDecorative = this.getBooleanAttribute('is-decorative', true);
    const isStandalone = this.hasAttribute('is-standalone');
    const isLinkWrapped = this.hasAttribute('is-link-wrapped');
    const title = this.getAttribute('title');

    if (!isDecorative) {
      if ((!isStandalone && !isLinkWrapped) || (isStandalone && isLinkWrapped)) {
        console.warn('IconElement: non decorative icons must either be standalone OR wrapped by a link.');
        return;
      }

      if (isStandalone && !title) {
        console.warn('IconElement: standalone non decorative icons must have a title.');
        return;
      }
    }

    const svgElement = document.createElementNS(SVG_NAMESPACE, 'svg');
    svgElement.setAttribute('xmlns', SVG_NAMESPACE);
    svgElement.setAttribute('fill', 'none');
    svgElement.setAttribute('viewBox', '0 0 24 24');
    svgElement.setAttribute('stroke-width', '1.5');
    svgElement.setAttribute('stroke', 'currentColor');

    const className = this.getAttribute('class');
    if (className) {
      svgElement.setAttribute('class', className);
    }

    if (isDecorative || isLinkWrapped) {
      svgElement.setAttribute('aria-hidden', 'true');
    } else {
      svgElement.setAttribute('role', 'img');

      const titleElement = document.createElementNS(SVG_NAMESPACE, 'title');
      titleElement.textContent = title;
      svgElement.append(titleElement);
    }

    const pathElement = document.createElementNS(SVG_NAMESPACE, 'path');
    pathElement.setAttribute('stroke-linecap', 'round');
    pathElement.setAttribute('stroke-linejoin', 'round');
    pathElement.setAttribute('d', ICON_PATHS[type]);
    svgElement.append(pathElement);

    this.append(svgElement);
  }

  private getBooleanAttribute(attributeName: string, defaultValue: boolean): boolean {
    if (!this.hasAttribute(attributeName)) {
      return defaultValue;
    }

    const value = this.getAttribute(attributeName);
    return value === '' || value === 'true';
  }
}

export function defineIconElement(): void {
  if (!customElements.get(ICON_ELEMENT_NAME)) {
    customElements.define(ICON_ELEMENT_NAME, IconElement);
  }
}

declare global {
  interface HTMLElementTagNameMap {
    [ICON_ELEMENT_NAME]: IconElement;
  }
}
